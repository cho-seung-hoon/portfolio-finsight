...
public class NewsApiService {
    ...
    public Mono<NewsApiResponseDTO> fetchNews(NewsApiRequestDTO requestDTO) {
        ...
        return webClient.get()
                ...
                .map(response -> {
                    List<NewsApiResponseDTO.NewsData> origin = response.getData();
                    if (origin == null || origin.isEmpty()) {
                        log.warn("News API 데이터가 null이거나 비어있습니다.");
                        return response;
                    }
                    ...
                    // esg != null 인 기사만 필터링
                    List<NewsApiResponseDTO.NewsData> filteredOrigin = origin.stream()
                            .filter(data -> data.getEsg() != null)
                            .collect(Collectors.toList());

                    List<NewsApiResponseDTO.NewsData> processedData = new ArrayList<>();

                    for (NewsApiResponseDTO.NewsData data : filteredOrigin) {
                        // 정규화
                        ...
                        processedData.add(NewsApiResponseDTO.NewsData.builder()
                                .id(data.getId())
